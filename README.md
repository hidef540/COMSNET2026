...
# Feedback-Driven Data Placement for
Deadline-Aware Cloud Right-Sizing
## 📁 Directory Structure
...
Contains all the datasets used in the experiments.
  ...
  - `input.txt`: Generated synthetic dataset formatted acoording to CRED approach
   ...
   - 
- **User Request/**
  - Each User job `j` in `trace.txt` and `google.txt` contains 5 fields:
...
- In line `188`, of `main.cpp` change file name from `trace.txt` to `google.txt` 

In order to simulate synthetic data
- In line `188`, of `main.cpp` change file name from `google.txt` to `trace.txt`
